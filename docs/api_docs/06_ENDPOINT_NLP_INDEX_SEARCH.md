...
**System Collections:** In addition to the user project collection, the search also scans the system-reserved collections `collection_1` through `collection_5` and merges results by score.
...
```python
collection_name = f"{settings.VECTOR_DB_COLLECTION_PREFIX}{project.project_id}"
collection_names = system_collection_names + [collection_name]
```
...
```python
results = vectordb_client.search_by_vector_multi_collection(
